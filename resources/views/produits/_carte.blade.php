<article class="bg-fond rounded-2xl p-4 shadow-sm" id="produit-{{ $produit->id }}" data-produit-carte="{{ $produit->id }}">
    ...
    <div class="mt-3 flex items-center gap-2 border-t border-fond-alterne pt-3">
        <a href="{{ route('produits.modifier', $produit) }}"
           class="inline-flex flex-1 items-center justify-center gap-1 px-3 py-2 text-sm font-medium text-principale rounded-lg hover:bg-fond-alterne">
        ...
        <form method="POST" action="{{ route('produits.disponibilite', $produit) }}"
            ...
            @method('PATCH')
            <button type="submit"
                    class="inline-flex w-full items-center justify-center gap-1 px-3 py-2 text-sm font-medium text-texte-secondaire rounded-lg hover:bg-fond-alterne">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        ...
        <form method="POST" action="{{ route('produits.destroy', $produit) }}"
            ...
            @method('DELETE')
            <button type="submit"
                    class="inline-flex items-center justify-center h-9 w-9 text-texte-secondaire rounded-lg hover:text-alerte hover:bg-fond-alterne"
                    title="Supprimer">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
