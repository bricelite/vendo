<x-app-layout>
    ...
    @if (! $boutique)
        <div class="bg-fond rounded-2xl shadow-sm p-6 text-center">
    ...
    @else
        ...
        <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            <a href="{{ route('produits.index') }}" class="bg-fond rounded-2xl p-4 shadow-sm">
            ...
            <a href="{{ route('commandes.index') }}" class="bg-fond rounded-2xl p-4 shadow-sm">
        ...
        <div class="mt-4 grid grid-cols-2 gap-3">
            <a href="{{ route('produits.creer') }}"
               class="inline-flex items-center justify-center gap-2 px-4 py-3 bg-accent text-white font-semibold text-sm rounded-lg shadow-sm">
            ...
            <a href="{{ route('boutique-publique.accueil', $boutique) }}" target="_blank"
               class="inline-flex items-center justify-center gap-2 px-4 py-3 border border-principale text-principale font-semibold text-sm rounded-lg">
                Voir ma boutique
            </a>
        </div>

        {{-- Partager la boutique --}}
        <section class="mt-4 bg-fond rounded-2xl p-4 shadow-sm" x-data="partageLien()">
            ...
            <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <button type="button" @click="copier()"
                            class="inline-flex items-center gap-1.5 px-3 py-2.5 bg-principale text-white font-semibold text-sm rounded-lg"
                    ...
                    <a href="https://wa.me/?text={{ rawurlencode("Découvrez ma boutique {$boutique->nom} !\n" . url(route('boutique-publique.accueil', $boutique))) }}"
                       ...
                       class="inline-flex items-center gap-1.5 px-3 py-2.5 bg-[#25D366] text-white font-semibold text-sm rounded-lg"
            ...
            <p class="mt-3 text-sm text-principale bg-fond-alterne rounded-lg px-3 py-2 truncate">{{ url(route('boutique-publique.accueil', $boutique)) }}</p>
        </section>
