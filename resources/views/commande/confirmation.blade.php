<x-boutique-layout :boutique="$commande->boutique">
    ...
    <div class="py-8 text-center" x-data x-init="$store.panier.vider()">
        ...
        {{-- Récap des articles --}}
        <div class="mt-5 glass-solid p-4 text-left max-w-sm mx-auto">
        ...
        <div class="mt-6 glass-subtle rounded-xl p-4 text-left text-sm text-texte-secondaire">
            <p class="mb-1">
        ...
        <a href="https://wa.me/{{ $telephoneWhatsApp }}?text={{ rawurlencode($messageWhatsApp) }}"
           ...
           class="mt-4 inline-flex items-center justify-center gap-2 w-full max-w-sm py-3 border border-succes text-succes font-semibold text-sm rounded-xl">
        ...
        @if (session('succes_avis'))
            <div class="mt-4 bg-succes/10 text-succes text-sm rounded-xl px-4 py-3">
                {{ session('succes_avis') }}
            </div>
        @endif
    ...
    <div class="mt-2" x-data="{ ouvert: false }">
        <button type="button" @click="ouvert = !ouvert"
                class="w-full flex items-center justify-between glass-solid p-4">
